...
void record()
{
    joint0[arrayStep] = ist[3]; // write positions in servo array
    joint1[arrayStep] = ist[0];
    joint2[arrayStep] = ist[1];
    joint3[arrayStep] = ist[2];
}
void Read()
{
...
void move_servo()
{       
  servo_0.writeMicroseconds(ist[0]); // send milissecond values to servos
...
void play_servo()
{
    ...
    if (steps < stepsMax) // sure we not reach the end from a move
    {
      ...
      {
        time = del*3;    // time is the delay in microsecns we wait in the mainloop until
      }
      else if(steps == 80) 
      {
        time = del*2;    // a micro step will be done
      }
      else if(steps == 100)
      {
        time = del-1;    // cannot explain here is not del*1
      }
      
      if(steps == stepsMax-200) 
      {
        time = del*2;        // stop ramp down (200 microsteps before end time will be increased
      }
      ...
      servo_0.writeMicroseconds(ist[0]); // Zange //anschlüsse gemappt!
      servo_1.writeMicroseconds(ist[1]); // Hand
      servo_2.writeMicroseconds(ist[2]); // Schulter
      servo_3.writeMicroseconds(ist[3]); // Ellbogen
    }
    else
    {
